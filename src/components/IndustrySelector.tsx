      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {INDUSTRIES.map(({ id, label, description, icon: Icon, regBadge }, i) => (
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="group text-left p-4 bg-[#0f2040] border border-[#1e3a6e] rounded-xl hover:border-blue-600/50 hover:bg-[#112248] transition-colors focus:outline-none focus:ring-2 focus:ring-blue-600/50 flex items-stretch gap-0"
          >
            {/* Left accent bar */}
            ...
            <div className="flex-1 min-w-0">
              ...
              <span className="inline-block mt-2.5 text-[10px] font-bold px-2 py-0.5 bg-blue-600/10 text-blue-400 border border-blue-600/20 rounded uppercase tracking-wider">
                {regBadge}
              </span>
            </div>
          </motion.button>
